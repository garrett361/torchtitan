...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.score_before_experts:
            top_scores = torch.empty_like(top_scores_experts_sorted)
            top_scores[token_indices_experts_sorted] = top_scores_experts_sorted
            # Save an allocation: store unsorted outputs in routed_input
            ...
            out_experts = (
                torch.bmm(top_scores.float(), routed_input.float()).to(x.dtype).squeeze(1)
            )
            if out is None:
            ...
            out = out.scatter_add(
                dim=0,
                index=token_indices_experts_sorted // self.router.top_k,
                src=routed_output,
            )
            out = out.reshape(bs, slen, dim)
        return out
