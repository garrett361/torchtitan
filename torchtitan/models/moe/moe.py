...
class MoEClone(MoE):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.score_before_experts:
            # Unsort scores and routed outputs.
            top_scores_experts_sorted[token_indices_experts_sorted] = (
                top_scores_experts_sorted.clone()
            )
            routed_output[token_indices_experts_sorted] = routed_output.clone()
            routed_output = routed_output.reshape(-1, self.router.top_k, dim)
            top_scores_experts_sorted = top_scores_experts_sorted.reshape(
                -1, 1, self.router.top_k
            )
            out_experts = (
                torch.bmm(top_scores_experts_sorted, routed_output.float())
                .to(x.dtype)
                .squeeze(1)
            )
        else:
            routed_output[token_indices_experts_sorted] = routed_output.clone()
            out_experts = routed_output.reshape(-1, self.router.top_k, dim).sum(dim=1)
...
class MoEEmptyLike(MoE):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.score_before_experts:
            # Unsort scores and routed outputs.
            top_scores_experts_unsorted = torch.empty_like(top_scores_experts_sorted)
            top_scores_experts_unsorted[token_indices_experts_sorted] = (
                top_scores_experts_sorted
            )
            routed_output_unsorted = torch.empty_like(routed_output)
            ...
            top_scores_experts_unsorted = top_scores_experts_unsorted.reshape(
                -1, 1, self.router.top_k
            )
            out_experts = (
                torch.bmm(top_scores_experts_unsorted, routed_output_unsorted.float())
                .to(x.dtype)
                .squeeze(1)
            )
        else:
